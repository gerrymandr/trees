...
mutable struct Node
    ...
    function Node(i::Int)::Node # for Terminal Nodes
        return new(NodeEdge(i, i), Vector{UInt8}(), Vector{UInt8}(), 0, Set{ForbiddenPair}(), Vector{UInt8}([]), true)
    end
    ...
    function Node(root_edge::NodeEdge, base_graph::SimpleGraph)::Node
        ...
        return new(root_edge, Vector{UInt8}(), comp_weights, 0, Set{ForbiddenPair}(), comp_assign, true)
    end

    function Node(label::NodeEdge, comp::Vector{UInt8}, comp_weights::Vector{UInt8},
                  cc::UInt8, fps::Set{ForbiddenPair}, comp_assign::Vector{UInt8}, deadend::Bool)::Node
